...
model_choice = st.sidebar.radio(
    "Choose LLM backend:",
    options=["Gemini (Cloud API)", "Finetuned Model (vLLM)"],
    index=0,
    key="model_choice",
)
use_model = "gemini" if model_choice.startswith("Gemini") else "finetuned"
...
try:
    ...
    st.sidebar.warning("⚠️ Could not reach backend to switch model.")
