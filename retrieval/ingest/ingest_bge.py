"""
Ingest corpus.csv into Qdrant Cloud — BGE-m3 collection (dense + sparse).

Usage:
    CUDA_VISIBLE_DEVICES=0 python ingest_bge.py --csv ../data/corpus.csv --batch-size 64
"""
import argparse
import json
import os
...
def create_collection_if_needed(client: QdrantClient, collection_name: str):
    """Create the BGE hybrid collection if it doesn't already exist."""
...
def ingest(csv_path: str, batch_size: int = 64):
    ...
    create_collection_if_needed(client, BGE_COLLECTION)
    ...
    start_from = load_checkpoint(BGE_COLLECTION)
    ...
    for _batch_num, i in enumerate(_batch_indices, 1):
        ...
        embs = model.encode(texts, return_dense=True, return_sparse=True, return_colbert_vecs=False)
        dense_vecs = embs["dense_vecs"]
        sparse_weights = embs["lexical_weights"]

        points = []
        for j, (text, infor_id, c_id) in enumerate(batch):
            indices = list(sparse_weights[j].keys())
            values = list(sparse_weights[j].values())
            points.append(
                models.PointStruct(
                    id=str(uuid.uuid4()),
                    vector={
        ...
        client.upsert(collection_name=BGE_COLLECTION, points=points)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest corpus into BGE-m3 Qdrant collection")
